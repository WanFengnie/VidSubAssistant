...
def _normalize_drop_paths(files) -> list[str]:
    """windnd may pass bytes or str; Chinese paths need gbk/mbcs on Windows."""
    out: list[str] = []
    for f in files or []:
...
本工具本身不会自动下载安装 FFmpeg、CUDA 工具包或 Ollama。
缺少时需自行安装，并遵守各软件的许可证与使用条款。

会自动获取的内容（仅在开始识别且本地没有缓存时）：
• Whisper 语音模型权重（经 faster-whisper / Hugging Face）
  — 版权与许可以各模型页面为准（常见为 MIT 等宽松许可）
  — 下载与使用即表示你接受模型作者与 Hugging Face 的条款
  — 本程序不拥有、不转售这些模型文件

不会自动下载：
• FFmpeg（多为 GPL/LGPL，请自行安装）
• NVIDIA CUDA / 驱动 / 运行库（NVIDIA 专有许可；可选，缺则用 CPU）
• Ollama 与本地大模型、云翻译 API（由你自行安装或申请）

你处理的视频内容版权由你自行负责；请确保有权转写与翻译。
识别/翻译结果可能有误，仅供参考使用，重要用途请人工校对。

详细列表可参阅程序目录中的 THIRD_PARTY_NOTICES.md。"""
...
WHISPER_MODELS = [
    ...
    ("small（中弱）", "small"),
    ("medium（中）", "medium"),
    ("large-v3-turbo（强）", "large-v3-turbo"),
    ("large-v3（最强）", "large-v3"),
]
QUALITY_PRESETS = [
    ("快速（弱）", "fast"),
    ("均衡（中）", "balanced"),
...
PROMPT_MENU_PLACEHOLDER = "（使用记录）"
PROMPT_MENU_CLEAR = "清空"
PROMPT_HISTORY_MAX = 24
OLLAMA_NO_MODEL = "（无模型）"
...
class App(ctk.CTk):
    def __init__(self) -> None:
        ...
        self._save_after_id: str | int | None = None
        self._queue: list[dict] = []
        self._queue_running = False
        self._prompt_history: list[str] = []
        self._prompt_menu_map: dict[str, str] = {}
        ...
        if paths:
            self.after(
                50,
                lambda: self._log(
                    f"[路径] 导出={paths.get('export_dir')} | 模型缓存={paths.get('model_cache_dir')}"
                ),
            )
    ...
    def _build_ui(self) -> None:
        ...
        self.video_entry = ctk.CTkEntry(
            ...
            placeholder_text="可拖入视频到窗口任意处，也可点浏览…",
        )
        ...
        self.enqueue_banner_var = ctk.StringVar(value="入队设置：请先在下方选择源语言 / 目标语言（可加提示词）")
        ...
        self.prompt_template_menu = ctk.CTkOptionMenu(
            ...
            values=[PROMPT_MENU_PLACEHOLDER, PROMPT_MENU_CLEAR],
            command=self._on_prompt_template,
            width=200,
        )
        self.prompt_template_menu.grid(row=0, column=0, sticky="w")
        ctk.CTkButton(
            prompt_bar,
            text="保存模板",
            width=72,
        ...
        self.youdao_key_var = ctk.StringVar()
        self.youdao_secret_var = ctk.StringVar()
        self.azure_key_var = ctk.StringVar()
        self.azure_region_var = ctk.StringVar(value="global")
        ...
        ctk.CTkEntry(fr, textvariable=self.baidu_id_var).grid(row=0, column=1, sticky="ew", pady=4, padx=6)
        ctk.CTkLabel(fr, text="密钥").grid(row=1, column=0, sticky="w", pady=4, padx=6)
        ctk.CTkEntry(fr, textvariable=self.baidu_key_var, show="*").grid(
        ...
        fr = ctk.CTkFrame(self.backend_host, fg_color="transparent")
        fr.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(fr, text="AppKey").grid(row=0, column=0, sticky="w", pady=4, padx=6)
        ctk.CTkEntry(fr, textvariable=self.youdao_key_var).grid(row=0, column=1, sticky="ew", pady=4, padx=6)
        ctk.CTkLabel(fr, text="AppSecret").grid(row=1, column=0, sticky="w", pady=4, padx=6)
        ctk.CTkEntry(fr, textvariable=self.youdao_secret_var, show="*").grid(
            row=1, column=1, sticky="ew", pady=4, padx=6
        )
        self._backend_frames["youdao"] = fr

        fr = ctk.CTkFrame(self.backend_host, fg_color="transparent")
        fr.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(fr, text="Key").grid(row=0, column=0, sticky="w", pady=4, padx=6)
        ctk.CTkEntry(fr, textvariable=self.azure_key_var, show="*").grid(
            row=0, column=1, sticky="ew", pady=4, padx=6
        )
        ctk.CTkLabel(fr, text="Region").grid(row=1, column=0, sticky="w", pady=4, padx=6)
        ctk.CTkEntry(fr, textvariable=self.azure_region_var).grid(row=1, column=1, sticky="ew", pady=4, padx=6)
        self._backend_frames["azure"] = fr
        ...
        ctk.CTkButton(btn_row, text="保存设置", width=90, command=self._save_settings_ui).grid(
        ...
        self._jobs_win: ctk.CTkToplevel | None = None
    ...
    def _apply_settings_to_ui(self) -> None:
        ...
        self.youdao_key_var.set(s.get("youdao_app_key", ""))
        self.youdao_secret_var.set(s.get("youdao_app_secret", ""))
        self.azure_key_var.set(s.get("azure_api_key", ""))
        self.azure_region_var.set(s.get("azure_region", "global") or "global")
    ...
    def _refresh_prompt_menu(self) -> None:
        """Rebuild dropdown from user prompt_history (no built-in templates)."""
        labels = [PROMPT_MENU_PLACEHOLDER, PROMPT_MENU_CLEAR]
        mapping: dict[str, str] = {}
        used: set[str] = set(labels)
        for text in self._prompt_history:
            base = self._shorten_prompt_label(text) or "（空）"
            label = base
            n = 2
            while label in used:
                suffix = f" ·{n}"
                label = (base[: max(1, 36 - len(suffix))] + suffix) if len(base) + len(suffix) > 40 else base + suffix
                n += 1
            used.add(label)
            labels.append(label)
            mapping[label] = text
    ...
    def _remember_prompt(self, text: str, *, persist: bool = True) -> bool:
        """Save a special prompt into user templates (newest first). Returns True if saved."""
        t = (text or "").strip()
        if not t:
            return False
        hist = [x for x in self._prompt_history if x != t]
        hist.insert(0, t)
        self._prompt_history = hist[:PROMPT_HISTORY_MAX]
        self._refresh_prompt_menu()
        if persist and getattr(self, "_settings_ready", False):
            self._persist_settings(quiet=True)
        return True

    def _save_prompt_template(self) -> None:
        """Manually save the current special-prompt text as a user template."""
        text = self._get_special_prompt()
        if not text:
            messagebox.showinfo("保存模板", "当前提示词为空，请先填写再保存。")
            return
        existed = text in self._prompt_history
        ok = self._remember_prompt(text, persist=True)
        if not ok:
            return
        # force persist even if settings not marked ready yet
        if not getattr(self, "_settings_ready", False):
            self._persist_settings(quiet=True)
        label = self._shorten_prompt_label(text)
        if existed:
            self._log(f"[提示词] 模板已更新到列表顶部：{label}")
            messagebox.showinfo("保存模板", f"该模板已在列表中，已移到最前：\n\n{label}")
        else:
            self._log(f"[提示词] 已保存模板（共 {len(self._prompt_history)} 条）：{label}")
            messagebox.showinfo(
                "保存模板",
                f"已保存到「使用记录」下拉列表（共 {len(self._prompt_history)} 条）。\n\n{label}",
            )
    ...
    def _on_prompt_template(self, choice: str = "") -> None:
        ...
        if name in ("", PROMPT_MENU_PLACEHOLDER, "（选择模板）"):
            return
        if name == PROMPT_MENU_CLEAR or name == "清空":
            self._set_special_prompt("")
            self.prompt_template_var.set(PROMPT_MENU_PLACEHOLDER)
            self._refresh_enqueue_banner()
            return
        body = self._prompt_menu_map.get(name, "")
        if body:
            self._set_special_prompt(body)
        self.prompt_template_var.set(PROMPT_MENU_PLACEHOLDER)
        self._refresh_enqueue_banner()
    ...
    def _collect_settings(self) -> dict:
        ...
        hist = list(self._prompt_history) if getattr(self, "_prompt_history", None) else []
        if not hist and isinstance(prev.get("prompt_history"), list):
            hist = [str(x).strip() for x in prev["prompt_history"] if str(x).strip()][:PROMPT_HISTORY_MAX]
        return {
            ...
            "youdao_app_key": self.youdao_key_var.get().strip(),
            "youdao_app_secret": self.youdao_secret_var.get().strip(),
            "azure_api_key": self.azure_key_var.get().strip(),
            "azure_region": self.azure_region_var.get().strip() or "global",
    ...
    def _wire_settings_autosave(self) -> None:
        ...
        vars_to_watch = [
            ...
            self.baidu_key_var,
            self.youdao_key_var,
            self.youdao_secret_var,
            self.azure_key_var,
            self.azure_region_var,
            self.openai_url_var,
            self.openai_key_var,
    ...
    def _refresh_enqueue_banner(self) -> None:
        ...
        if src in ("", "auto", "detect"):
            ...
            self.enqueue_banner_var.set("入队设置：请选择「目标语言」，或将翻译设为「不翻译」")
        else:
    ...
    def _install_drag_drop(self) -> None:
        ...
        try:
            ...
            if ok:
                self._log(
                    f"[拖放] 已启用（安全模式）：拖入视频到窗口后松开即可（{ok} 个区域）"
                )
            else:
        ...
        def work() -> None:
            ...
            def apply() -> None:
                ...
                if not info.get("ffmpeg_ok"):
                    self._log(
                        "[许可] 未检测到 FFmpeg：请自行安装并加入 PATH（不会自动下载），"
                        "并遵守 FFmpeg 官方许可条款。"
                    )

                msg = (info.get("cuda_message") or "").strip()
                if st == "ready":
                    self._log(f"[GPU] {msg or 'NVIDIA CUDA 可用'}")
                    self._apply_cuda_model_recommendation(silent=False)
                elif st == "need_runtime":
                    self._log("[GPU] 已检测到 N 卡，缺少 CUDA 12 运行库 — 请自行安装（不自动下载）")
                    if msg:
                    ...
                    if not getattr(self, "_gpu_need_runtime_warned", False):
                        ...
                        messagebox.showwarning(
                            ...
                            msg
                            or (
                                "已检测到 N 卡，但缺少 CUDA 12 运行库。\n"
                                "请自行 pip 安装 nvidia-cublas-cu12 等（详见日志）。\n"
                                "本程序不会自动下载。"
                            ),
                        )
                else:
                    self._log("[GPU] 该显卡未适配 NVIDIA CUDA，将使用 CPU")
                    if msg:
                        for line in msg.splitlines():
                            if line.strip():
                                self._log(f"[GPU] {line.strip()}")
                    if not getattr(self, "_gpu_no_nvidia_warned", False):
                        ...
                        tip = msg or (
                            "未检测到可用的 NVIDIA（N 卡）CUDA。\n"
                            "本程序 GPU 识别仅支持 N 卡（不支持 AMD/核显加速）。\n"
                    ...
                    # CPU 路线：弱机默认即可
                    cur = self._normalize_whisper_model(
                        self._map_label(self.model_label_var.get(), WHISPER_MODELS)
                    )
                    if cur in ("large-v3", "large-v3-turbo") and not getattr(
                        self, "_cpu_model_tip_shown", False
                    ):
                        self._cpu_model_tip_shown = True
                        self._log(
                            f"[模型] 无可用 GPU 时 large 系会很慢，建议改用 medium（中）或 small（中弱）"
                        )
    ...
    def _apply_cuda_model_recommendation(self, *, silent: bool = False) -> None:
        """N 卡可用时：把过时的弱默认升到 large-v3-turbo + 高质量（5080 等消费级推荐）。"""
        ...
        # 仅自动提升「旧默认 / 偏弱」档，不覆盖用户已选手动 large-v3 等
        weak_models = {"tiny", "base", "small", "medium", "distil-large-v3"}
        weak_quality = {"fast", "balanced"}
        changed = []
        if cur_m in weak_models:
            self._set_whisper_model_code(_RECOMMENDED_CUDA_MODEL)
            changed.append(f"模型 → {_RECOMMENDED_CUDA_MODEL}（强）")
        if cur_q in weak_quality:
            self._set_quality_code(_RECOMMENDED_CUDA_QUALITY)
            changed.append(f"质量 → 高质量（强）")
        ...
        if changed:
            ...
            if not silent:
                self._log(
                    "[模型] 已按消费级 N 卡优化："
                    + "；".join(changed)
                    + "。极限准确可选手动 large-v3（最强）+ 完整（最强）。"
                )
        elif not silent:
            self._log(
                f"[模型] 当前 {cur_m} / {cur_q}。"
                f"推荐：large-v3-turbo（强）+ 高质量；5080 等亦可 large-v3（最强）+ 完整。"
            )
    ...
    def _stop(self) -> None:
        ...
        try:
            ...
            pass
        # queue flag cleared when worker finishes

    def _purge_done_jobs_quiet(self) -> None:
        """Remove finished/empty jobs/* folders left behind (Windows lock / old bug)."""
        try:
            n = purge_finished_job_dirs()
            if n:
                self._log(f"[清理] 已删除完成/空任务目录 {n} 个")
        except Exception:
            pass
    ...
    def _open_jobs_window(self) -> None:
        if self._jobs_win is not None:
            try:
                ...
                self._jobs_win = None
        self._jobs_win = JobsWindow(self)
    ...
    def _resume_job_dict(self, job: dict) -> None:
        ...
        det = (job.get("detected_lang") or "").lower()
        if self._map_label(self.src_lang_label_var.get(), SOURCE_LANG_OPTIONS) in ("", "auto"):
            for label, code in SOURCE_LANG_OPTIONS:
                if code and det and (det.startswith(code) or code.startswith(det[:2])):
                    self.src_lang_label_var.set(label)
                    break
        if self._map_label(self.tgt_lang_label_var.get(), LANG_OPTIONS) == "":
            for label, code in LANG_OPTIONS:
                if code == "zh-CN":
                    self.tgt_lang_label_var.set(label)
                    break
        if self._map_label(self.src_lang_label_var.get(), SOURCE_LANG_OPTIONS) in ("", "auto"):
            messagebox.showinfo("续跑", "请选择源语言后将开始续跑。")
            return
    ...
    def _resume_last(self) -> None:
        ...
        if self._map_label(self.src_lang_label_var.get(), SOURCE_LANG_OPTIONS) in ("", "auto"):
            messagebox.showinfo("续跑", "请确认源语言/目标语言后，将自动开始续跑。")
            ...
            def progress_cb(message: str, percent: float) -> None:
                ...
                def ui() -> None:
                    ...
                    if not self._stopping:
                        ...
                        self.status_var.set(f"{head} · {message}")
                        self.queue_progress_var.set(
                ...
                for item in list(self._queue):
                    ...
                    self.after(
                        ...
                        lambda v=name, s=item["source_language"], t=item["target_language"], i=slot, has_p=bool(sp): (
                            ...
                            self._log(
                                f"—— 队列 {i}/{batch_total}: {v} | {s}→{t} | 提示词{'有' if has_p else '无'} ——"
                            ),
                        ),
                    )
                    try:
                        ...
                        self.after(0, lambda p=srt_p: self._log(f"字幕: {p}"))
                        if vid_p:
                            self.after(0, lambda p=vid_p: self._log(f"视频: {p}"))
                        ...
                        fail_n += 1
                        self.after(0, lambda err=str(e): self._log(f"[队列失败] {err}"))
                    self.after(0, self._refresh_queue_list)
    ...
    def _validate_global_keys(self) -> bool:
        ...
        if translator == "baidu" and (
            ...
            return False
        if translator == "youdao" and (
            not self.youdao_key_var.get().strip() or not self.youdao_secret_var.get().strip()
        ):
            messagebox.showerror("缺少密钥", "请填写有道 AppKey 与 AppSecret")
            return False
        if translator == "azure" and not self.azure_key_var.get().strip():
            messagebox.showerror("缺少 Key", "请填写 Azure Key")
            return False
        if translator == "openai" and not self.openai_key_var.get().strip():
            messagebox.showerror("缺少 Key", "请填写 API Key")
            return False
        return True
    ...
    def _start(self, force_resume: bool = False) -> None:
        ...
        target_language = self._map_label(self.tgt_lang_label_var.get(), LANG_OPTIONS)
        if source_language in ("", "auto", "detect"):
        ...
        self._remember_prompt(special_now)
        save_settings(self._collect_settings())
        self._apply_path_fields_to_runtime()
        ...
        def work() -> None:
            try:
                ...
                def done() -> None:
                    ...
                    self.video_var.set("")
                    self._log(f"字幕: {result.srt_path}")
                    if result.resumed_from:
                        self._log(f"曾从阶段续跑: {result.resumed_from}")
                    if result.output_video:
                        self._log(f"视频: {result.output_video}")
                    self._log(f"检测语言: {result.source_language}")
                    msg = f"处理完成！\n\n字幕: {result.srt_path}"
                    if result.output_video:
...
class JobsWindow(ctk.CTkToplevel):
    ...
    def __init__(self, app: App) -> None:
        ...
        ctk.CTkLabel(
            top,
            text="失败或中断的任务会保留进度，可续跑或删除",
            anchor="w",
        ).grid(row=0, column=0, sticky="w")
        ctk.CTkButton(top, text="刷新", width=70, command=self._reload).grid(row=0, column=1, padx=4)
    ...
    def _add_row(self, index: int, job: dict) -> None:
        ...
        if status != "done":
            ctk.CTkButton(btns, text="续跑", width=70, command=do_resume).pack(pady=2)
        ctk.CTkButton(btns, text="删除", width=70, fg_color="#8B2942", hover_color="#6B1F32", command=do_delete).pack(
            pady=2
        )
        ctk.CTkButton(btns, text="目录", width=70, command=do_folder).pack(pady=2)
